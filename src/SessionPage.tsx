...
export function SessionPage() {
  ...
  async function download() {
    setError('');
    try {
      const records = await allPages<Attendance>(`/attendance-sessions/${id}/attendance`);
      const cell = (value: string) => '"' + (/^[\s]*[=+@-]/.test(value) ? "'" + value : value).replace(/"/g, '""') + '"';
      const table = [['Curso', 'Clase', 'Alumno', 'Correo', 'Fecha y hora', 'Origen', 'Estado', 'Motivo'], ...records.map(r => [session!.course_name, session!.class_name, r.name, r.email, r.checked_in_at, r.source === 'QR' ? 'QR' : 'Manual', r.status === 'PRESENT' ? 'Presente' : 'Anulada', r.reason || ''])];
      const url = URL.createObjectURL(new Blob(['\uFEFF' + table.map(row => row.map(cell).join(';')).join('\r\n')], { type: 'text/csv;charset=utf-8;' }));
      const a = document.createElement('a'); a.href = url; a.download = `asistencias-${id}.csv`; a.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
